...
def extraire_zones_itineraire(etapes: list[dict]) -> dict:
    """Retourne les zones et les modes à partir d'étapes ordonnées d'un itinéraire."""
    if not etapes:
    ...
    return {
        "origine": etapes[0]["origine"],
        "destination": etapes[-1]["destination"],
        "zones_traversees": zones_traversees,
        "modes": modes,
    }


def devis_depuis_itineraire(
    etapes: list[dict],
    deplacements: int = 1,
    profil: str = "ordinaire",
    dans_24_heures: bool = False,
) -> dict:
    """Produit un devis STM-REM à partir d'étapes d'itinéraire déjà déterminées."""
    itineraire = extraire_zones_itineraire(etapes)
    devis = devis_trajet(
        origine=itineraire["origine"],
        destination=itineraire["destination"],
        zones_traversees=itineraire["zones_traversees"],
        deplacements=deplacements,
        profil=profil,
        dans_24_heures=dans_24_heures,
    )
